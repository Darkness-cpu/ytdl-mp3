...
const apiKeyUsage: Map<string, number> = new Map(apiKeys.map((key) => [key, 0]));
...
interface Stats {
  cachedItems: number;
  apiKeyUsage: Array<{ key: string; count: number }>;
}
...
app.get('/download', async (req: Request, res: Response) => {
  ...
  if (!url || typeof url !== 'string') {
    return res.status(400).json({ error: 'Invalid URL parameter' } as DownloadResponse);
  }

  const videoId: string | false = youtube_parser(url);
  if (!videoId) {
    return res.status(400).json({ error: 'Invalid YouTube URL' } as DownloadResponse);
  }

  // Check Cache
  if (cache.has(videoId)) {
    return res.json({ success: true, cached: true, link: cache.get(videoId) } as DownloadResponse);
  }

  const apiKey: string = getNextApiKey();
  ...
  try {
    ...
    if (link) {
      ...
      return res.json({ success: true, cached: false, link } as DownloadResponse);
    } else {
      throw new Error('Invalid response from API');
    }
  } catch (error: any) {
    console.error(error);
    return res.status(500).json({ error: 'Failed to fetch download link' } as DownloadResponse);
  }
});
